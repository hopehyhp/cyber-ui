        ...
        <aside class="controls card">
          ...
          <div style="margin-top:1rem">
            <div style="display:flex; gap:0.5rem; margin-bottom:0.5rem">
              <button class="btn ghost" :class="{ active: currentTheme === 'dark' }" @click="setTheme('dark')">黑色</button>
              <button class="btn ghost" :class="{ active: currentTheme === 'light' }" @click="setTheme('light')">白色</button>
            </div>
          </div>
        </aside>
      </div>
...
export default {
  ...
  data() {
    return {
      currentTheme: typeof localStorage !== 'undefined'
...
.home-hero { background: transparent; padding: 2rem 0 1rem 0; display: flex; align-items: center; }
...
@keyframes glitch-anim-1 {
  0% { clip-path: inset(5% 0 95% 0); }
  10% { clip-path: inset(8% 0 25% 0); }
...
@keyframes glitch-anim-2 {
  0% { clip-path: inset(95% 0 5% 0); }
  10% { clip-path: inset(75% 0 8% 0); }
