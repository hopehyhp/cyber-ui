<template>
  <button
    :class="[
      ...
      `cbtn--${type}`,
      `cbtn--${size}`,
      { disabled, 'is-plain': plain, 'is-round': round, 'is-icon-only': isIconOnly },
    ]"
  ...
  >
    ...
    <span v-if="!isIconOnly" class="content">
      <slot>{{ label }}</slot>
    </span>
  </button>
</template>
...
.cbtn.is-icon-only.cbtn--small {
  padding: 6px;
}

.cbtn.is-icon-only.cbtn--large {
  padding: 10px;
}

.cbtn.is-icon-only.is-round {
  padding: 8px;
  border-radius: 999px;
}
